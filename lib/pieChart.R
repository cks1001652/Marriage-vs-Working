...
#   rownames(stats) = c("1", "2", "3", "4")
  ...
  legend(1, 8*iniR, c("Type1,Both in Labor Force ", "Type2,Husband in Labor Force", "Type3,Wife in Labor Force", "Type4,Neither in Labor Force"), col=colors,pch=19, bty='n', ncol=1)
  # addtable2plot(-4.5, -2.5, stats, bty="n", hlines=FALSE, vlines=FALSE, title="The stats", display.colnames = TRUE, display.rownames = TRUE)
  text(-0.5,-8*iniR, "Labor Force Participation of Married-couple Household in the US")
