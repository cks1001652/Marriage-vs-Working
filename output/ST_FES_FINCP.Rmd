...
```{r}
source("../lib/readIn.R")
hus <- readIn_house(T,c("ST","FES","FINCP"),"../../ss13husa.csv","../../ss13husb.csv")
```
...
There are 666,947 missing values in __FES__, and our subset will have 617,090 families.  
...
fesnumbers <- summarise(hus,count=n())
fesnumbers <- left_join(fesnumbers,fescodes,by.x=c("FES"))
fesnumbers <- mutate(fesnumbers, percent = (count / sum(fesnumbers$count)) * 100)
